...
<div id="content">
	...
	<?php $post = $posts[0]; // Hack. Set $post so that the_date() works. ?>
		<?php /* If this is a category archive */ if (is_category()) { ?>				
 		...
 		<?php /* If this is a daily archive */ } elseif (is_day()) { ?>
			<h1 class="pagetitle"><?php _e('Archive for','andreas09'); ?> <?php the_time('F jS, Y'); ?></h1>
	...
	<?php while (have_posts()) : the_post(); ?>
		<div class="post" id="post-<?php the_ID(); ?>">
			...
			<p class="date"><?php _e('Posted by','andreas09') ?> <?php if (get_the_author_url()) { ?><a href="<?php the_author_url(); ?>"><?php the_author(); ?></a><?php } else { the_author(); } ?> <?php _e('on','andreas09') ?> <?php the_time(__('jS F Y','andreas09')) ?></p>
