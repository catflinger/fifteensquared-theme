...
<h2><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php _e('Permanent Link to','andreas09'); ?> <?php the_title(); ?>"><?php the_title(); ?></a></h2>
		...
		<p class="date"><?php _e('Posted by','andreas09'); ?> <?php if (get_the_author_url()) { ?><a href="<?php the_author_url(); ?>"><?php the_author(); ?></a><?php } else { the_author(); } ?> <?php _e('on','andreas09'); ?> <?php the_time (__('F jS, Y','andreas09')) ?></p>

		<div class="entrytext">
<img src="<?php get_bloginfo('url'); ?>http://fifteensquared.net/sandbox/wp-content/images/authors/<?php the_author_ID();?>.jpg" class="auth" alt="<?php the_author(); ?>. " title="<?php the_author(); ?> "/>
			...
			<p class="postmetadata">
				 		<?php _e('This entry was posted on','andreas09'); ?> <?php the_time(__('l, F jS, Y','andreas09')) ?> 
